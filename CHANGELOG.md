...
- **Carrés vides à la place de certains caractères** (coche `✓`, croix `✘`, flèches `→ ← ↑ ↓`, retour `⏎`, touche `␣`) : les polices par défaut ne les couvrent pas, ce qui sautait aux yeux en collant du texte venu d'ailleurs. Inter, déjà embarquée pour les noms d'auteur, sert désormais de repli — sans un octet de plus — et **Noto Sans Symbols 2** (OFL-1.1) est ajoutée pour le reste : la couverture des symboles passe de 1 005 à 1 827 caractères sur la plage U+2000–U+2BFF
