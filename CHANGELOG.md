...
- **Carrés vides à la place de certains caractères** (coche `✓`, flèches `→ ← ↑ ↓`, retour `⏎`) : les polices par défaut ne les couvrent pas, ce qui sautait aux yeux en collant du texte venu d'ailleurs. Inter, déjà embarquée pour les noms d'auteur, sert désormais de repli — sans un octet de plus. Une tâche cochée d'une liste Markdown était touchée elle aussi (`☑`, qu'aucune police embarquée ne dessine)
