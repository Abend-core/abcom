...
- **Carrés vides à la place de certains caractères** (coche `✓`, flèches `→ ← ↑ ↓`, retour `⏎`) : les polices par défaut ne les couvrent pas, ce qui sautait aux yeux en collant du texte venu d'ailleurs. Inter, déjà embarquée pour les noms d'auteur, sert désormais de repli — sans un octet de plus — et **Noto Sans Symbols 2** (OFL-1.1) est ajoutée pour le reste : la couverture des symboles passe de 524 à 1 303 caractères sur la plage U+2000–U+2BFF. Une tâche cochée d'une liste Markdown était touchée elle aussi (`☑`, qu'aucune police embarquée ne dessine — elle est rendue par `✓`)
