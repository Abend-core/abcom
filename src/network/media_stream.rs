...
async fn stream_out(job: &MediaSendJob, event_tx: &Sender<AppEvent>) -> std::io::Result<()> {
    ...
    if job.header.requires_ack {
        ...
        if !accepted {
            // Refus (ou absence de réponse) : on signale l'interruption.
            let _ = event_tx.send(failed(id, total)).await;
            return Ok(());
        }
    }
...
mod tests {
    ...
    async fn streams_a_file_end_to_end() {
        // Serveur sur un port éphémère ; l'émetteur vise `port - 2` (il ajoute +2).
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let port = listener.local_addr().unwrap().port();

        let media_dir =
            std::env::temp_dir().join(format!("abcom_ms_{}_{:?}", std::process::id(), std::thread::current().id()));
        ...
        // Le fichier doit avoir été écrit à l'identique dans media/<id>.
        assert_eq!(std::fs::read(media_dir.join("test.bin")).unwrap(), payload);

        // On doit avoir reçu l'annonce puis une progression terminée.
