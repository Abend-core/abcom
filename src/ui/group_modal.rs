        ...
        egui::Window::new("Créer un groupe")
            ...
            .show(ctx, |ui| {
                    ...
                    if all_peers.is_empty() {
                        ...
                        for peer in &all_peers {
                            ...
                            if ui.checkbox(&mut is_selected, peer).changed() {
                                if is_selected { self.group_members_selected.insert(peer.clone()); }
                                else { self.group_members_selected.remove(peer); }
                            }
                        }
                    }
                });
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.add_enabled(is_valid, egui::Button::new("✓ Créer")).clicked() {
                        ...
                        if let Some(group) = self.state.lock().unwrap().create_group(name, members) {
                            ...
                            for addr in online_peers {
                                let _ = self.send_group_tx.try_send(SendGroupRequest { to_addr: addr, event: create_event.clone() });
                            }
